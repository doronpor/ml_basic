...
class ConvLayer:
    ...
    def im2col(
        ...
        X_col = np.zeros(
            (batch_size, h_out * w_out, self.kernel_size * self.kernel_size * channels)
        )
    ...
    def forward(self, X: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
        ...
        out = X_col @ W_col + self.b.reshape(
            1, -1
    ...
    def backward(
        ...
        for i in range(batch_size):
            ...
            dW += dW_i.reshape(
                self.kernel_size, self.kernel_size, self.in_c, self.num_filters
            )
